...
async def send_prompt_to_eval_engine(message: str, task: str, conversation_id: str):
    ...
    """
    logger.info(f"[send_prompt_to_eval_engine] Sending prompt for validation")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"[send_prompt_to_eval_engine] Validation failed with status code {response.status_code}")
            logger.error(f"[send_prompt_to_eval_engine] Error response: {response.text}")
            result = None
    return result

async def send_response_to_eval_engine(response: str, task: str, inference_id: str, context: list[LLMMessage]):
    """
    ...
    """
    logger.info(f"[send_response_to_eval_engine] Sending response for validation")
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"[send_response_to_eval_engine] Validation failed with status code {response.status_code}")
            logger.error(f"[send_response_to_eval_engine] Error response: {response.text}")
            result = None
    return result
...
def get_eval_engine_model(entity_type: str, entity_name: str, config: dict) -> str:
    ...
    logger.debug(f"[get_eval_engine_model] Getting eval engine task for {entity_type}.{entity_name}")
    ...
    try:
        ...
        if eval_engine_model == "":
            logger.error(f"[get_eval_engine_model] Eval engine model not found for {entity_type}.{entity_name}")
            logger.error(f"[get_eval_engine_model] Returning default eval engine model ['default']['eval_engine_model']")
            return config['tools']['default']['eval_engine_model']
        
        logger.debug(f"[get_eval_engine_model] Found eval engine model: {eval_engine_model}")
        return eval_engine_model
    except KeyError:
        logger.error(f"[get_eval_engine_model] Entity {entity_name} not found in {entity_type}")
...
def load_eval_engine_config(config_path: str = "eval_engine_config.json") -> dict:
    ...
    """
    logger.info(f"[load_eval_engine_config] Loading eval engine configuration from {config_path}")
    try:
        ...
        logger.error(f"[load_eval_engine_config] Eval engine configuration file not found at {config_path}")
        raise
    except json.JSONDecodeError:
        logger.error(f"[load_eval_engine_config] Invalid JSON in eval engine configuration file")
        raise