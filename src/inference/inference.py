...
class InferenceResult:
    ...
    def return_pii(self) -> bool:
        ...
        for rule in self.rule_results:
            if (
                "pii" in rule.name.lower()
                or "personally identifiable information" in rule.name.lower()
            ):
                logger.debug(
                    f"[InferenceResult.return_pii] Found PII rule: {rule.name}"
                )
                if not rule.result_boolean:
                    logger.info(
                        f"[InferenceResult.return_pii] PII rule failed: {rule.name}"
                    )
                    return False
        logger.debug(
            "[InferenceResult.return_pii] All PII rules passed or no PII rules found"
        )
        return True
    ...
    def return_hallucination(self) -> bool:
        ...
        """
        logger.debug(
            "[InferenceResult.return_hallucination] Checking hallucination rules"
        )
        if self.rule_results is None:
        ...
        for rule in self.rule_results:
            if "hallucination" in rule.name.lower():
                logger.debug(
                    f"[InferenceResult.return_hallucination] Found hallucination rule: {rule.name}"
                )
                if not rule.result_boolean:
                    logger.info(
                        f"[InferenceResult.return_hallucination] Hallucination rule failed: {rule.name}"
                    )
                    return False
        logger.debug(
            "[InferenceResult.return_hallucination] All hallucination rules passed or no hallucination rules found"
        )
        return True
