...
from src.arthur_eval_engine.helpers import get_eval_engine_model, send_prompt_to_eval_engine, send_response_to_eval_engine
...
class SoloOrchestratorAssistantAgent(RoutedAgent):
    ...
    def __init__(
        ...
        self._orchestrator_task = get_eval_engine_model("agents", "OrchestratorAgent", self._config)
    ...
    async def handle_message(self, message: UserTextMessage, ctx: MessageContext) -> None:
        ...
        eval_engine_response = await send_prompt_to_eval_engine(message.content, self._orchestrator_task, conversation_id)
        inference_result = InferenceResult(eval_engine_response)
        logger.debug(f"[SoloOrchestratorAssistantAgent] Arthur Evaluation Engine validation response: {inference_result.get_rule_details()}")
        logger.debug(f"[SoloOrchestratorAssistantAgent] Arthur Evaluation Engine validation response: {inference_result.get_pass_fail_results()}")
        await self._model_context.add_message(SystemMessage(content=f"Arthur Evaluation Engine validations: {inference_result.get_pass_fail_string()}", source=message.source))
        ...
        eval_engine_message = await send_response_to_eval_engine(final_resolution_response.content, self._orchestrator_task, inference_result.get_inference_id(), context)
    ...
    async def message_loop(self, message: UserTextMessage, ctx: MessageContext, system_message: SystemMessage, loop_count: int, conversation_id: str) -> None:
        ...
        logger.debug("[SoloOrchestratorAssistantAgent.message_loop] Getting context for Arthur Evaluation Engine")
        context = await self._model_context.get_messages()
    ...
    async def validate_tool_responses(self, tool_responses: list[dict], message: str, context: list[LLMMessage], conversation_id: str) -> bool:
        ...
        for tool_response in tool_responses:
            ...
            validation_task = get_eval_engine_model("tools", tool_response["name"], self._config)
            ...
            eval_engine_response = await send_prompt_to_eval_engine(message, validation_task, conversation_id)
            inference_result = InferenceResult(eval_engine_response)
            ...
            if eval_engine_response is not None:
                inference_result = InferenceResult(eval_engine_response)
                logger.debug(f"[ToolValidation] Arthur Evaluation Engine validation response: {inference_result.get_rule_details()}")
                logger.debug(f"[ToolValidation] Arthur Evaluation Engine validation response: {inference_result.get_pass_fail_results()}")
